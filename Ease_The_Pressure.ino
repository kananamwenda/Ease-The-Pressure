...
Ease The Pressure : By Patricia Mwenda
...
int analogPin = A1;    // select the input pin for the potentiometer
...
void loop() {
  ...
  if (len > 1) { // if the sensor is activated
    for (int i = 0; i < len; i++) { // loop through until how "hard" the user was pushing.
      CircuitPlayground.setPixelColor(i,random(255), random(255), random(255)); // set the lights to white
